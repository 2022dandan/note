    ...
    <div class="background-collect">
        <div class="title">
            <div style="display: flex; margin-left: 50px;width: 400px;" >
                ...
                <span style="font-size:30px ;padding-left: 10px; padding-top: 12px;">我的收藏夹</span> 
                <!-- <Search></Search>   -->
            </div>
        </div>
        ...
        <el-col :span="5" v-for="(form) in cardForm" :key="'note-card' + form.card_name"  style="margin-top: 20px;">
            <CardShow @getCollectCard="getCollectCard"  :originForm="form"></CardShow>
        </el-col>
        </el-row>
        ...
        <el-col :span="4" v-for="(form) in noteForm" :key="'note' + form.note_name"  style="margin-top: 20px;">
            <NoteShow @getCollectNote="getCollectNote"  :originForm="form" ></NoteShow>
        </el-col>
        </el-row>
    </div>
    ...
    export default {
        ...
        methods: {
            async getCollectCard() {
                ...
                info.card_collect = 1
                // console.log(info, "cooo")
                const res = await request('http://localhost:8000/note/getCollectNoteCard', 'post', info)
                if (res?.code === 0) {
                ...
                console.log(this.cardForm, "收藏卡片列表")
                }
                else{
                console.log("所有卡片查找失败")
                }
            },
            async getCollectNote() {
                // 展示所有笔记的逻辑
                const info = {}
                info.user_name = sessionStorage.getItem('username')
                info.note_collect = 1
                console.log(info, 123)
                const res = await request('http://localhost:8000/note/getCollectNote', 'post', info)
                if (res?.code === 0) {
                    this.noteForm = res.result.map(item => {
                        item.note_type = JSON.parse(item.note_type)
                        return item
                    })
                    console.log(this.noteForm,'notecooo')
                }
                else{
